...
export default function DashboardNew() {
  ...
  return (
    <>
      <Head>
        <title>Tetsuta Creative Lab — Dashboard New</title>
        <meta name="viewport" content="width=device-width, initial-scale=1" />
      </Head>

      {/* 背景グラデーション */}
      <div className="min-h-screen bg-gradient-to-b from-indigo-100 via-sky-50 to-white">
        {/* ヘッダー */}
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {modules.map((m) => (
                <Link
                  key={m.title}
                  href={m.href}
                  className="group rounded-xl border bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md hover:border-indigo-300"
                >
